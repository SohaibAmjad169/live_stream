...
import { X, TrendingUp, DollarSign, Calendar, Users, Edit3, Check, AlertCircle } from "lucide-react";
...
export default function StreamDetailsModal({
  ...
  const currentData = activeTab === "direct" ? directSalesData : breaksData;
          ...
          <div className="text-[#0B0B58] p-4 sm:px-6 pt-6 pb-0 rounded-t-2xl">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex-1">
                <h2 className="text-xl sm:text-2xl font-bold mb-2">Stream Details</h2>
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-[#19191D] text-sm">
                  ...
                  <span className={`inline-flex px-3 py-1 text-xs font-medium rounded-full border ${getStatusColor(stream.status)}`}>
              ...
              <div className="flex gap-2">
                ...
                {(stream.status === 'draft' || stream.status === 'denied') && !isEditing && (
                  <button
            ...
            {stream.status === 'denied' && (
              <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                <p className="text-sm text-red-700">
                  <strong>Report Denied:</strong> Please review and edit your report before resubmitting.
                </p>
              </div>
            )}
            ...
            {isInReview && (
              <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <p className="text-sm text-yellow-700">
                  <strong>Under Review:</strong> This report is currently being reviewed by an admin.
                </p>
              </div>
            )}
            ...
            {isApproved && (
              <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                <p className="text-sm text-green-700">
                  <strong>Approved:</strong> This report has been approved and counts towards your payroll.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="p-4 sm:px-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
              <div className="bg-[#F9FAFB] border border-[#C3D3E2] rounded-xl p-4 sm:p-5">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-xs sm:text-sm font-medium text-[#1E3A8A] mb-1">TOTAL REVENUE</div>
                    <div className="text-2xl sm:text-2xl font-bold text-[#000000]">${totalRevenue}</div>
                  </div>
              ...
              <div className="bg-[#F9FAFB] border border-[#C3D3E2] rounded-xl p-4 sm:p-5">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-xs sm:text-sm font-medium text-[#1E3A8A] mb-1">TOTAL PROFIT</div>
                    <div className="text-2xl sm:text-2xl font-bold text-[#000000]">${totalProfit}</div>
                  </div>
                  <div className="w-10 h-10 sm:w-11 sm:h-11 bg-[#E5E7EB] rounded-xl flex items-center justify-center">
            ...
            <div className="h-[300px] bg-white border border-[#E5E7EB] rounded-xl overflow-hidden">
              ...
              <div className="bg-white border-b border-[#F0F0F0] px-5 py-3">
                <div className={`grid ${activeTab === 'direct' ? 'grid-cols-7' : 'grid-cols-6'} gap-4 py-2 text-sm font-bold text-[#1E3A8A] uppercase tracking-wider`}>
                  <div>Date & Time</div>
              ...
              <div className="max-h-80 overflow-y-auto custom-scrollbar">
                <div className="divide-y divide-[#E5E7EB]">
                  {currentData.map((transaction, index) => (
                    <div
                      key={transaction.id}
                      className={`px-5 py-3 grid ${activeTab === 'direct' ? 'grid-cols-7' : 'grid-cols-6'} gap-4 items-center hover:bg-[#F8F9FA] transition-all duration-200 group ${
                        index % 2 === 0 ? 'bg-white' : 'bg-[#F9FAFB]'
                      }`}
                    >
                      <div className="text-sm text-[#19191D] font-medium">
                        <div>{transaction.date}</div>
                        <div className="text-xs text-[#6B7280]">{transaction.time}</div>
                      </div>
                      {activeTab === 'direct' && (
                        <div className="text-sm text-[#19191D] font-medium">
                          {transaction.buyer || <span className="text-[#6B7280] italic">No buyer</span>}
                        </div>
                      )}
                      <div className="text-sm text-[#19191D] font-medium">{transaction.item}</div>
                      <div className="text-sm text-[#000000] font-bold">${transaction.price}</div>
                      <div className="text-sm text-[#19191D] font-medium">
                        <span className="bg-[#F9FAFB] border border-[#C3D3E2] px-2 py-1 rounded-full text-xs">
                          {transaction.qty}
                        </span>
                      </div>
                      <div className="text-sm text-[#1E3A8A] font-bold">${transaction.revenue}</div>
                      <div className="text-sm text-[#1E3A8A] font-bold">${transaction.profit}</div>
                    </div>
                  ))}
                </div>
              </div>
